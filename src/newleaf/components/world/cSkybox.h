...
struct CSkybox {
    ...
    explicit CSkybox(bool uf, glm::vec3&& fc, float fd, float fg, float rs)
      : enable_fog(uf), fog_color(std::move(fc)), fog_density(fd), fog_gradient(fg), rotation_speed(rs) {}

    void print() const {
      ENGINE_BACKTRACE("\t\tenable_fog: {0}\n\t\t\t\t\t\tfog_color: "
                       "{1}\n\t\t\t\t\t\tfog_density: {2}\n\t\t\t\t\t\tfog_gradient: "
                       "{3}\n\t\t\t\t\t\trotation_speed: {4}",
                       enable_fog, glm::to_string(fog_color), fog_density, fog_gradient, rotation_speed);
    }
