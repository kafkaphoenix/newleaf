...
struct CTime {
    ...
    void set_time(float time) {
      ...
      currentMinute =
        static_cast<uint32_t>((seconds / 60.f) - (currentHour * 60.f));
...
template <>
inline void nl::SceneManager::on_component_added(entt::entity e, components::CTime& c) {
  c.validate();
  c.set_time(c.startingTime);

  m_registry.replace<components::CTime>(e, c);
}