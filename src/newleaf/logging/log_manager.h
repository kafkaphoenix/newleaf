...
class LogManager {
  public:
    ...
    static spdlog::logger& get_engine_logger() { return *m_engine_logger; }
    static spdlog::logger& get_app_logger() { return *m_app_logger; }
    static spdlog::logger& get_engine_backtrace_logger() { return *m_engine_backtrace_logger; }
    static spdlog::logger& get_app_backtrace_logger() { return *m_app_backtrace_logger; }
    static std::string_view get_engine_logger_level() {
      return std::string_view(spdlog::level::to_string_view(m_engine_logger->level()));
    }
    static std::string_view get_app_logger_level() {
      return std::string_view(spdlog::level::to_string_view(m_app_logger->level()));
    }
    static std::string_view get_engine_logger_flush_level() {
      return std::string_view(spdlog::level::to_string_view(m_engine_logger->flush_level()));
    }
...
#define ENGINE_TRACE(...) ::nl::LogManager::get_engine_logger().trace(__VA_ARGS__)
#define ENGINE_DEBUG(...) ::nl::LogManager::get_engine_logger().debug(__VA_ARGS__)
#define ENGINE_INFO(...) ::nl::LogManager::get_engine_logger().info(__VA_ARGS__)
#define ENGINE_WARN(...) ::nl::LogManager::get_engine_logger().warn(__VA_ARGS__)
#define ENGINE_ERROR(...) ::nl::LogManager::get_engine_logger().error(__VA_ARGS__)
#define ENGINE_CRITICAL(...) ::nl::LogManager::get_engine_logger().critical(__VA_ARGS__)
...
#define APP_CRITICAL(...) ::nl::LogManager::get_app_logger().critical(__VA_ARGS__)

#define ENGINE_BACKTRACE(...) ::nl::LogManager::get_engine_backtrace_logger().debug(__VA_ARGS__)
#define APP_BACKTRACE(...) ::nl::LogManager::get_app_backtrace_logger().debug(__VA_ARGS__)