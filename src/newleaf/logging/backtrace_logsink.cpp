...
BacktraceLogsink::BacktraceLogsink(std::string&& path) : m_path(std::move(path)) {}
...
void BacktraceLogsink::clear(std::string_view source) {
  ...
  m_records.erase(
    std::remove_if(m_records.begin(), m_records.end(), [&](const Record& record) { return record.m_source == source; }),
    m_records.end());
  lock.unlock();
}
...
void BacktraceLogsink::sink_it_(const spdlog::details::log_msg& msg) {
  ...
  r.m_time = std::format("{:%D %T}", std::chrono::floor<std::chrono::seconds>(time));
